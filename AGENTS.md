...
Agno can also **ask**. `UserFeedbackTools` puts one `ask_user` tool on the leader: a structured question with two to four options, for the case where the answer changes what Agno would do and cannot be read from the entities, the notes, or the session. It earns its place under the growth rule because it increases what people tell Agno — an ambiguous "what's happening here?" becomes two named candidates to pick between rather than a paragraph of guesses. It is the same HITL machinery as Platform Builder's archive gate, so the run pauses and resumes through the AgentOS UI, Slack, or `continue_run` over MCP. **One consequence to keep in view: the Agno team is now a component that can pause**, which makes it a poor schedule target — a scheduled run pauses at 09:00 with nobody to answer. Platform Builder's instructions tell it to read a target's tools before scheduling and refuse one carrying `user_feedback_tools`, but that is a prompt rule, not a code guard: probed on 3.0.0a5 it created the schedule anyway on the first attempt and refused on the second. Treat it as a strong default, not a guarantee, and do not schedule the Agno team.
