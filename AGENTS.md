...
| [`agents/chief.py`](agents/chief.py) | The team mascot, now the team lead — a `Team` with LearningMachine (profile, memory, entities in agentic mode) + FileSystem notes + web tools on the leader, and Agent Builder, Platform Manager, plus every Studio-built agent as members (resolved fresh per run); the Slack default. |
...
- **Learning team lead** — see [`agents/chief.py`](agents/chief.py). Chief is a `Team`: direct tools (the notes toolkit — the leader sees each tool individually) plus `learning=` (the LearningMachine attaches its stores' tools, guidance, and recall automatically), and a **callable members factory** resolved per run (`cache_callables=False`) that folds in every Studio-built agent beside the reference members. The same tools+learning shape works unchanged on a plain `Agent` when nothing needs delegating; use `learning=` whenever the component should accumulate durable state across sessions.
