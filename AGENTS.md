...
- `app.registry.registry` exposes the safe Studio registry Agent Builder can use: Agno docs MCP, web search, `agent_files` (a per-agent private file store — its templated namespace resolves to the wielding agent's own id, so every built agent that carries it gets an isolated space with its own quota), a send-scoped Slack toolkit (when `SLACK_BOT_TOKEN` is set), utility functions, the default model, the shared DB, and the platform-manager reference agent. The registry is **sealed** (`sealed=True`): AgentOS never folds OS-registered components or their wiring back in at boot, so this declaration is exactly what builders can wire — Chief (whose members include the builder itself), agent-builder, and the privileged runtime toolkits (`studio`, Chief's `filesystem` notes and `agent_runner` dispatch surface, the `agentos` ops toolkit) are mechanically outside builds and schedule targets, not just instructed out. Missing capability requests route to a scoped code change, never to an escape hatch.
