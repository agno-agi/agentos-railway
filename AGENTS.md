...
| [`app/knowledge.py`](app/knowledge.py) | `shared_knowledge` — the platform's own PgVector base (operator content), registry name `shared-knowledge`. Ships empty; the AgentOS UI's Knowledge page is the load path. A product agent gets a dedicated base of its own (`create-agent`'s product pattern), never this one. |
