...
| [`app/learning.py`](app/learning.py) | `shared_self` — the per-user profile/memory machine the three platform agents carry and the registry offers to built components under the name `user-self`. |
| [`app/knowledge.py`](app/knowledge.py) | `platform_knowledge` — the one PgVector base, registry name `platform-knowledge`. Ships empty; the AgentOS UI's Knowledge page is the load path. |
...
| [`app/notes.py`](app/notes.py) | The shared `brain` notebook, declared once: Agno mounts the full toolkit, the registry offers the scoped `shared_notes` functions to built components. |
