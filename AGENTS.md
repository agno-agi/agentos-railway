...
| [`app/ingest.py`](app/ingest.py) | `KnowledgeManagementTools(knowledge=product_knowledge)` — `ingest_url(url, page_cap)` (sitemap discovery with indexes followed, Parallel Extract when `PARALLEL_API_KEY` is set, page-by-page `WebsiteReader` otherwise, one row per page with its source URL) and `list_ingested_sources`. Mounted on Platform Builder only, never in the registry: ingestion is an operator action. A placeholder for agno 3.0.3's `KnowledgeManagementTools` (`specs/agno/website-ingestion`) — same class name, so the swap is an import change and `ingest_url` collapses to one insert call. |
