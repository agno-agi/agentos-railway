...
**It is deliberately on the leader only, and not in the registry.** A component that can pause is a bad schedule target: the run stops at 09:00 with nobody to answer. Agno is the one component that is essentially always interactive — Slack, the UI and MCP are all people typing — and it is not a thing anyone schedules, since the platform's scheduled work is deterministic workflows. A *built* component is the opposite: scheduling one is a normal thing to ask Platform Builder for, and probing found it doing exactly that, putting an agent whose only tool was `ask_user` on an enabled 15-minute cron. So the capability stays where a human is always present and stays out of the palette where it is not. Platform Builder still refuses to schedule the Agno team on the same grounds — but that is a prompt rule rather than a code guard, so treat it as a strong default and do not schedule Agno. Putting `ask_user` back in the registry is a reviewed code change, and worth making once the framework refuses to schedule a target whose tools can pause.
