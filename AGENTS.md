...
| [`app/registry.py`](app/registry.py) | Safe Studio registry used by Agent Builder — docs MCP, web tools, utility functions, the platform-manager reference agent. |
| [`app/config.yaml`](app/config.yaml) | UI manifest per component (keyed by `id`): description + quick prompts. |
| [`agents/chief.py`](agents/chief.py) | The team mascot and team lead — a `Team` with LearningMachine (profile, memory, entities in agentic mode) + FileSystem notes + web tools + a run-scoped `agent_runner` (StudioRunnerTools) on the leader, and Agent Builder + Platform Manager as members; runs every Studio-built agent on demand; the Slack default. |
...
- **Learning team lead** — see [`agents/chief.py`](agents/chief.py). Chief is a `Team`: direct tools (the notes toolkit — the leader sees each tool individually) plus `learning=` (the LearningMachine attaches its stores' tools, guidance, and recall automatically), static reference members, and a run-scoped **StudioRunnerTools** mount (`agent_runner`) that lists and runs every Studio-built agent on demand — dispatch stays O(1) in prompt and DB cost however many agents the team builds. The same tools+learning shape works unchanged on a plain `Agent` when nothing needs delegating; use `learning=` whenever the component should accumulate durable state across sessions.
