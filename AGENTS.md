...
| [`app/ingest.py`](app/ingest.py) | `ProductIngestTools` — `ingest_product_docs(url, page_cap)` (sitemap discovery with indexes followed, Parallel Extract when `PARALLEL_API_KEY` is set, page-by-page `WebsiteReader` otherwise) and `list_product_sources`. Mounted on Platform Builder only, never in the registry: ingestion is an operator action. |
