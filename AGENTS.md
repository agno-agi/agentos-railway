...
Agno can also **ask**. `UserFeedbackTools` puts one `ask_user` tool on the leader: a structured question with two to four options, for the case where the answer changes what Agno would do and cannot be read from the entities, the notes, or the session. It earns its place under the growth rule because it increases what people tell Agno — an ambiguous "what's happening here?" becomes two named candidates to pick between rather than a paragraph of guesses. It is the same HITL machinery as Platform Builder's archive gate, so the run pauses and resumes through the AgentOS UI, Slack, or `continue_run` over MCP. **One consequence to keep in view: the Agno team is now a component that can pause.** Platform Builder already refuses to schedule a component carrying the ask-the-user toolkit, and reads the target before scheduling it, so a scheduled Agno run is refused at the point it would be created rather than hanging at 3am.
