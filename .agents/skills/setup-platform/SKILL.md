...
**The product brief** you hand create-agent when they name a product — complete, so it builds without asking more: the product pattern (Step 3 there), ingest with `app/tools.py`'s `ingest_url` inside the container (Parallel Extract if `PARALLEL_API_KEY` is set, else the per-page route — same rows and citations, slower; a fresh clone lands here), page cap 50, `knowledge=product_knowledge`, `learning=shared_learning`, instructions you write that carry the grounding rules in create-agent's product pattern, no other tools; three smoke probes.
