...
Wire it the way [`teams/lead.py`](../../../teams/lead.py) does: `FileSystem(get_postgres_db(), namespace="radar")`, then `tools=[fs.tools(), WebSearchTools(), was_reported, record_reported]`. The file tools are what let it manage the preferences file and the brief notes; the instructions say the ledger is only ever touched through the two functions.
