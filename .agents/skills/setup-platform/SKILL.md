...
**The product brief** you hand create-agent when they name a product — complete, so it builds without asking more: product pattern, dedicated base, sitemap discovery (follow a sitemap index), page cap 50, one row per page; Parallel Extract if `PARALLEL_API_KEY` is set, else the per-page WebsiteReader route (same rows and citations, slower — a fresh clone lands here); knowledge search as the only tool, no `learning=`, the instruction template as written; three smoke probes.
